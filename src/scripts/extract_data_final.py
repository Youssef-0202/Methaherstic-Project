...
# Counter for assigning unique sub-groups to TD/TP sessions
td_subgroup_counter = defaultdict(lambda: defaultdict(int))

def extract_groups_from_session(session_name, session_type, group_header, room_id, day, start_time):
    """
    Extract group information with intelligent sub-group inference.

    For TD/TP sessions without explicit sub-groups, we create unique sub-groups
    based on the room to avoid conflicts.
    """
    session_name = session_name.strip()

    # Pattern 1: TC S1/S2/S3 sessions with explicit sub-groups
    ...
    if main_group_match:
        ...
        if subgroup_patterns:
            ...
            # No explicit sub-group found
            if session_type in ["TD", "TP"]:
                # For TD/TP without explicit sub-groups, create unique sub-groups
                # Use room as differentiator
                key = f"{main_group}_{session_type}"

                # Assign a unique sub-group number based on room
                if room_id not in td_subgroup_counter[key]:
                    td_subgroup_counter[key][room_id] = len(td_subgroup_counter[key]) + 1

                subgroup_num = td_subgroup_counter[key][room_id]
                return [f"{main_group}_TD{subgroup_num}"]
            else:
                # For Cours, use main group
                return [main_group]
...
def assign_teacher_smart(session_name, session_type, room_id, day, start_time, teacher_counter):
    ...
    if session_type in ["TD", "TP"]:
        # For TD/TP: different rooms at same time = different teachers
        key = f"{course_base}_{session_type}_{day}_{start_time}_{room_id}"
    else:
        # For Cours: same course = same teacher
...
def process_data_final():
    print(f"Processing {INPUT_FILE} with final intelligent extraction...")
            ...
            if val and str(val).strip():
                ...
                if min_col in room_col_map:
                    ...
                    if min_row in row_time_map:
                        ...
                        # Extract groups with intelligent inference
                        groups = extract_groups_from_session(session_name, session_type, group_header, room_id, day, start_time)
    ...
    print(f"[OK] Created {len(df_groups)} group definitions.")
    ...
    td_subgroups = [g for g in all_groups_mentioned if '_TD' in g]

    print(f"\nSUMMARY:")
    print(f"  - Total assignments: {len(df_assignments)}")
    print(f"  - Unknown groups: {unknown_count} ({unknown_count/len(df_assignments)*100:.1f}%)")
    print(f"  - Sub-group assignments: {subgroup_count} ({subgroup_count/len(df_assignments)*100:.1f}%)")
    print(f"  - Auto-generated TD sub-groups: {len(td_subgroups)}")
    print(f"  - Total groups: {len(df_groups)}")
    print(f"  - Total teachers: {len(df_teachers)}")

    if td_subgroups:
        print(f"\n  Auto-generated sub-groups: {', '.join(sorted(td_subgroups))}")
